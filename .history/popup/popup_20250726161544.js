...
function refreshHotItems() {
  const button = document.getElementById('refresh-hot-items');
  ...
  chrome.runtime.sendMessage({ action: 'refreshHotItems' }, (response) => {
    if (response && response.success) {
      showNotification('Hot items refreshed successfully!');
      loadHotItems(); // Reload the display
    } else {
      showNotification('Failed to refresh hot items');
    }
